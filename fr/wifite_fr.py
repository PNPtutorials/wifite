...
if not NO_XSERVER:
	try:
		...
		NO_XSERVER=True
		print R+'[!] Erreur pendant le chargement de tkinter; '+O+' GUI d�sactiv� ...'
...
class App:
	...
	def __init__(self, master):
		...
		if lst == [] or len(lst) == 0:
			print GR+'[!] '+R+'Aucune carte Wifi n\'a �t� d�tect�e'
			print GR+'[!] '+O+'Soyez sur que votre carte wifi est bien branch�e, ensuite verifiez avec airmon-ng'
			sys.exit(0)
		elif len(lst) == 1:
			if lst[0].strip() == '':
				print GR+'[!] '+R+'Aucune carte Wifi n\'a �t� d�tect�e'
		...
		w=Checkbutton(frame, text='illimit�',variable=self.wepwendless, font=f0nt, activeforeground='red',\
		...
		w=Checkbutton(frame, text='illimit�',variable=self.wpawendless, font=f0nt, activeforeground='red',\
	...
	def lookup(self):
		file=tkFileDialog.askopenfile(parent=root, mode='rb',title='S�lectionne une liste de mot de passe')
		if file != None:
			self.dicttxt.delete(0, END)
	...
	def ifacelist(self):
		...
		if len(lst) == 0:
			print R+'[!] Aucun reseau wifi n\'a ete trouv�!'
			print R+'[!] Lancez airmon-ng et v�rifiez vos drivers wireless'
	...
	def doit(self, args):
		...
		for a in args:
			cmd.append(a)
		print GR+'[+] '+G+'Ex�cution: '+W+ './' + THEFILE+' ' + ' '.join(args)
		try:
			subprocess.call(cmd)
...
def get_revision():
	...
	if start != -1 and stop != -1:
		...
		try:
			...
			rev=rev.split('\n')[0]
			print R+'[+] Num�ro de r�vision invalide: "'+rev+'"'
...
def update():
	...
	try:
		print GR+'[+] '+W+'V�rification des mises a jour...'
		
		r,d,t=get_revision()
		
		r=int(r)
		if r == -1:
			print GR+'[+] '+R+'Impossible d\'acceder a code.google.com; mise � jour annul�e'
		
		elif r > REVISION:
			print GR+'[+] '+W+'Une '+G+'nouvelle version'+W+' de wifite.py est disponible! (r'+str(r)+')' 
			print GR+'[+] '+W+'changements:'+W
			print GR+'    -'+d.replace('\n','\n    -')
			print GR+'[+] '+W+'Mise � jour sortie: '+G+t+W+'\n'
			print GR+'[+] '+W+'Voulez vous '+G+'t�l�charger et mettre � jour'+W+' wifite.py? (y/n): '
			ans=raw_input()
			if ans.lower()[0] == 'y':
				upgrade()
			else:
				print GR+'[+] '+W+'Mise � jour '+O+'annul�e'+W+''
		
		elif r == REVISION:
			print GR+'[+] '+W+'Votre version de wifite.py est '+G+'� jour'
		
		else:
			print GR+'[+] '+W+'Vous avez en quelques sorte une '+G+'r�vision futuriste'+W+' de wifite.py'
	except KeyboardInterrupt:
		print R+'[+] ^C Interrompu; Mise � jour annul�e'
...
def upgrade():
	...
	"""
	print GR+'[+] '+G+'T�l�chargement'+W+' de la mise a jour...'
	sock = urllib.urlopen('http://wifite.googlecode.com/svn/trunk/fr/wifite_fr.py')
	page = sock.read()
	
	if page == '':
		print R+'[+] Impossible de t�l�charger wifite.py; Exit'
		return
	...
	print GR+'[+] '+G+'Mis � jour!'+W+' tapez "./'+THEFILE+'" pour relancer'
...
def main():
	...
	try:
		...
		if not check_root():
			print R+'[+] Vous devez �tre '+O+'root'+O+'!'
		...
		if len(sys.argv) > 1:
			...
			print GR+'[+] '+W+'D�marrage de l\''+G+'interface GUI'+W
			app = App(root)
			app
		...
		if had_wpa:
			...
			if DICT != '':
				...
				if CRACKED == 0:
					print R+str(CRACKED)+' crack�e'+W
				else:
					print G+str(CRACKED)+' crack�e'+W
		else:
		...
		# display the log
		if len(THE_LOG) > 0:
			print GR+'[+] '+G+'R�sum� de la session:'+W
			for i in THE_LOG:
				print GR+'    -'+i
			print W
...
def aircrack_warning():
	...
	if req != '':
		print GR+'[+] '+O+'ATTENTION:'+W+' des applications/packages '+G+'requis'+W+' n ont pas �t� trouv�:'+W+req
	if rec != '':
		print GR+'[+] '+O+'ATTENTION:'+W+' des applications/packages '+G+'recommand�s'+W+' n ont pas �t� trouv�'+W+rec
...
def check_intel():
	...
	for line in out.split('\n'):
		if line.find(IFACE) != -1 and line.find('Intel 4965') != -1:
			print GR+'[+] '+W+'chipset intel4965'+G+'d�tect�'+W+'\n'
			HAS_INTEL4965=True
			return
...
def banner():
	...
	print G+"':.  ':.  ':. "+GR+"/_\\"+G+" ,:'  ,:'  ,:'  "
	print G+" ':.  ':.    "+GR+"/___\\"+G+"    ,:'  ,:'   "+GR+"Con�u pour backtrack4"
	print G+"  ':.       "+GR+"/_____\\"+G+"      ,:'     "
	print G+"           "+GR+"/       \\"+G+"             "
	print W
...
def handle_args(args):
	...
	for a in args:
		...
		if a == '-no-color' or a == '--no-color' or a == '-nocolor':
			...
			C  = ""
			GR = ""
			print '[+] Les couleurs ont �t� neutralis�es :)\n'
			break
	...
	for a in args:
		...
		if a == 'h' or a == 'help' or a == '-h' or a == '?' or a == '/?' or a == 'commands':
			...
			print GR+'[+] '+W+'La r�vision actuelle de wifite est: '+G+'r'+str(REVISION)+W
			print GR+'[+] '+W+'Lancez '+G+'./wifite.py -upgrade'+W+' pour v�rifier les mises � jour'
			sys.exit(0)
	...
	while (i < len(args)):
		...
		if a == '-i' or a == '--iface':
			try:
				IFACE=args[i+1]
				print GR+'[+] '+W+'Interface wireless utilis�e: "'+G + IFACE + W+'"'
			except IndexError:
			...
			try:
				...
				if ESSID.lower() == 'all' or ESSID.lower() == '"all"':
					if ESSID.startswith('pow>'):
						print O+'[!] ESSIDS d�ja cibl�s avec une puissance sup�rieure � '+ESSID[4:]+'dB'+W
					else:
						ESSID = 'all'
			...
			if ESSID.startswith('pow>'):
				print O+'[!] ESSIDS d�ja cibl�s avec une puissance supp�rieure � '+ESSID[4:]+'dB'+W
			else:
				ESSID = 'all'
				print GR+'[+] '+W+'Ciblage essid "'+G + ESSID + W+'"'
			...
			try:
				...
				print R+'[!] Entrez -e pow>## où ## est un nombre de 1 - 2 chiffres'
				print R+'[!] exemple: ./'+THEFILE+' -e pow>55'
				print W
				subprocess.call(['rm','-rf',TEMPDIR])
				sys.exit(0)
			
			print GR+'[+] '+W+'Ciblage des reseaux avec un signal de puissance supp�rieure � '+G+ str(tempint)+'dB'+W
			...
			print GR+'[+] '+W+'Scan uniquement pour '+G+'R�seaux crypt�s WEP'+W
			WPA=False
			
		elif a == '-nowep' or a == '--no-wep':
			print GR+'[+] '+W+'Scan uniquement pour '+G+'R�seaux crypt�s WPA'+W
			WEP=False
			...
			CHANGE_MAC=False
			print GR+'[+] '+W+'Changement de votre MAC '+O+'D�sactiv�'+W
			
		elif a == '-mac' or a == '--change-mac':
			# keep this here, for the old-school users that still use '-mac' option
			print GR+'[+] '+W+'Changement de votre MAC '+G+'Activ�'+W
		
		elif a == '-noarp' or a == '--no-arp':
			WEP_ARP=False
			print GR+'[+] '+W+'Attaque arp-replay '+G+'D�sactiv�e'+W
		
		elif a == '-nochop' or a == '--no-chop':
			WEP_CHOP=False
			print GR+'[+] '+W+'Attaque chop-chop '+G+'D�sactiv�e'+W
		
		elif a == '-nofrag' or a == '--no-frag':
			WEP_FRAG=False
			print GR+'[+] '+W+'Attaque par fragmentation '+G+'D�sactiv�e'+W
		
		elif a == '-no0841' or a == '--no-p0841':
			WEP_P0841=False
			print GR+'[+] '+W+'Attaque -p 0841 '+G+'D�sactiv�e'+W
		
		elif a == '-console' or a == '--console':
			print GR+'[+] '+G+'mode console'+W+' Activ�e'
		
		elif a == '-f' or a == '--force-fake':
			print GR+'[+] '+W+'Continuez l\'attaque WEP si la fake-auth �choue '+O+'Activ�e'+W+''
			EXIT_IF_NO_FAKEAUTH=False
		
		elif a == '-nod' or a == '--no-deauth':
			print GR+'[+] '+W+'D�sauthentification des r�seaux cach�s '+O+'D�sactiv�'+W+''
			NO_HIDDEN_DEAUTH=True
			
		elif a == '-nostrip' or a == '--no-strip':
			print GR+'[+] '+W+'D�coupage du handshake WPA '+O+'D�sactiv�'+W+''
			STRIP_HANDSHAKE=False
		
		elif a == '-anon' or a == '--anonymous' or a == '--anon':
			ANONYMOUS_MAC=random_mac()
			print GR+'[+] '+W+'Adresse MAC anonyme '+G+'Activ�e'+W+''
		
		i += 1
		
	if WEP==False and WPA==False:
		print R+'[!] Erreur, WEP ET WPA sont desactives!'
		print R+'[!] Ce sont les seuls cat�gories de r�seaux que l\'ont peux cracker'
		print R+'[!] Le programme va se fermer'
		print W
		subprocess.call(['rm','-rf',TEMPDIR])
		sys.exit(0)
...
def halp(full=False):
	...
	if not full:
		print G+'  -help    '+GR+'Affiche l aide compl�te\n'
	...
	if full:
		...
		print '             \t Entrez manuellement le nom de votre interface si elle n est pas trouv�e automatiquement'
	...
	if full:
		...
		print '            \t Avant de commencer une attaque, votre adresse MAC est chang�e par une'
		print '            \t adresse MAC al�atoire. Apr�s que l\'attaque sois finie, l\'adresse MAC est restaur�e.\n'
	else:
	...
	if full:
		...
		print '             \t Le programme va le demander si tu ne l\'a pas pr�cis�'
		print '             \t Utilisez -d pour rentrer directement le chemin du fichier'
		print '             \t Par d�faut, il va aussi chercher "wordlis.txt" dans le meme dossier que wifite.py'
	...
	if full:
		...
		print '          \t D�fini le temps maximum d\'attente pour le handshake WPA (en minutes)'
		print '          \t Entrez "0" pour illimit�\n'
	else:
	...
	if full:
		...
		print '          \t D�fini le temps maximum d\'attente pour chaque attaque WEP.'
		print '          \t D�pend des param�tres, �a peux prendre du temps'
		print '          \t si le temps est 10min, chaque methode de crack prendra 10 min'
		print '          \t si vous avez 4 attaques WEP s�lectionn�e, �a peux prendre 40 min'
		print '          \t Entrez "0" pour illimit�\n'
	else:
		print G+'  -wepw\t   '+GR+'Temps maximum (en minutes) pour capturer/cracker la cl� WEP de chaque r�seau'
	#PPS
	if full:
		print G+'  --pps\t\t'+GR+'     ex. -pps 400'
		print '          \t Paquets Par Secondes; uniquement pour les attaques WEP.'
		print '          \t Plus de PPS = plus de IVs captur�s, Ce qui veux dire un crack plus rapide'
		print '          \t Choisisez un petit PPS pour un r�seau faible ou �loign�\n'
	else:
	...
	#wep: no-attack
	if full:
		print G+'  --no-arp\t'+GR+' D�sactive les attaques WEP arp-replay'
		print G+'  --no-chop\t'+GR+' D�sactive les attaques WEP chop-chop'
		print G+'  --no-frag\t'+GR+' D�sactive les attaques WEP par fragmentation'
		print G+'  --no-p0841\t'+GR+' D�sactive les attaques WEP -p0841\n'
	else:
		print G+'  -noarp   '+GR+'D�sactive les attaques arp-replay'
		print G+'  -nochop  '+GR+'D�sactive les attaques chop-chop'
		print G+'  -nofrag  '+GR+'D�sactive les attaques fragmentation'
		print G+'  -no0841  '+GR+'D�sactive les attaques -p0841'
	#WEP FORCE FAKE-AUTH (cancels attack if failed)
	if full:
		print G+'  --force-fake\t '+GR+'Pendant une attaque WEP, si la fake-auth �choue, forcer � continuer'
		print   '              \t avec les attaques fake-authentication'
		print   '              \t Par default il stop l\'attaque quand la fake-auth �choue\n'
	else:
		print G+'  -f       '+GR+'Force les attaques WEP a continuer si la fake-auth �choue'
	# SSID DEAUTH
	if full:
		#-nod --no-deauth
		print G+'  --no-deauth\t '+GR+'D�sactive la d�connexion des clients sur les r�seaux cach�s'
		print   '             \t par d�fault, wifite va d�connecter les clients connect�s sur un r�seau cach�'
		print   '             \t *wifite d�connecte les clients sur un SSID cach� en MODE CHANNEL FIXE*\n'
	else:
		print G+'  -nod     '+GR+'Ne pas d�connecter les SSID cach� pendant le scan d\'un channel fixe'
	#NO COLORS
	if full:
		print G+'  --no-color\t '+GR+'Ne pas afficher les couleurs aga�antes (utilise les couleurs systeme)\n'
	else:
		print G+'  -nocolor '+GR+'Ne pas utiliser du texte color� (utilise les couleurs systeme)'
	...
	if full:
		...
		print '             \t essid (nom) du point d\'acc�s (routeur)'
		print '             \t Ca force une attaque limit�e; pas d\'autres r�seaux ne serront attaqu�s\n'
		#print '             \t     e.g. -e "all"'
	...
	#ALL
	if full:
		print G+'  -all, --all\t'+GR+' Cible et attaque tout les r�seaux trouv�s'
		print '           \t ce qui est dangereux, car la plupart des attaques n�cessitent l\'injection,'
		print '           \t la plupart des cartes wifi ne peuvent pas injecter, sauf s\'ils sont � proximit� de l\'AP \n'
	else:
		print G+'  -all\t   '+GR+'Cible et attaque les points d\'acc�s trouv�s'
	#POWER
	if full:
		print G+'  -p, --power\t'+GR+'     ex. -p 55'
		print '             \t Puissance minimale (dB)'
		print '             \t Similaire � l\'option "-e all", sauf qu\'il filtre et ignore les APs'
		print '             \t qui sont trop �loign�s. \n'
	else:
		print G+'  -p\t   '+GR+'Filtre une puissance minimale (dB) pour attaquer; il ignore les reseaux faibles ou �loign�s'
	#CHANNEL
	if full:
		print G+'  -c, --channel\t'+GR+'     ex. -c 6'
		print '               \t Channel � scanner *uniquement*'
		print '               \t Scanne un seul channel'
		print '               \t A utiliser uniquement si vous connaissez LE channel a �couter\n'
	else:
		print G+'  -c\t   '+GR+'Channel a scanner (Par defaut, tout les channels)'
	#NOWPA
	if full:
		print G+'  --no-wpa\t'+GR+' Ignore tout les r�seaux WPA'
		print '          \t utile pour filtrer quand vous avez --power ou "-e all" \n'
	else:
		print G+'  -nowpa   '+GR+'NE PAS scanner les r�seaux WPA (ON par d�faut)'
	#NOWEP
	if full:
		print G+'  --no-wep\t'+GR+' Ignore tout les r�seaux WEP'
		print '          \t utile pour filtrer quand vous avez -p ou "-e all"\n'
	else:
		print G+'  -nowep   '+GR+'NE PAS scanner les r�seaux WEP (ON par d�faut)'
...
def find_mon():
	...
	if len(ifaces) == 0 or ifaces == []:
		...
		if len(poss) == 0:
			...
			print R+'[!] Vous avez peut-�tre besoin d\'installer de nouveaux drivers'
			print R+'[+] Le programme va s\'arreter.'
			print W
			subprocess.call(['rm','-rf',TEMPDIR])
			sys.exit(0)
			...
			print GR+'\n[+] '+W+'S�lectionnez le p�riph�rique que vous voulez mettre en mode moniteur:'
			...
			while err==True:
				try:
					print GR+'[+] '+W+'S�lectionnez l\'interface wifi(entre '+G+'1'+W+' et '+G + str(len(poss)) + W+'):'+G,
					num=int(raw_input())
...
def wpa_crack(index):
	...
	try:
				...
				if last != -1 and first != -1:
					...
					if ks.strip() != '':
						...
						if last != -1 and first != -1:
							...
							pmks=txt[first:last]
							if pmks.strip() != '':
								print G+str(pmks)+W+' Cl�s totales;',
		...
		print '\r'+GR+'['+sec2hms(time.time() - START_TIME)+'] '+W+'cracking;',
		print G+str(ks)+W+' C/s;',
		print G+str(total_pmks)+W+' cl�s totales;',
		print G+'100%'+W,
		...
		if os.path.exists(TEMPDIR+'wpakey.txt'):
			...
			print '\n'+GR+'['+sec2hms(time.time()-START_TIME)+'] '+G+'"' + ssid + '" Crack�! La cl� est: "'+C+cracked+G+'"'
			logit('cracked WPA key for "' + ssid + '", the key is: "' + cracked + '"')
			CRACKED += 1
			
		else:
			print GR+'\n['+sec2hms(time.time()-START_TIME)+'] '+W+'Le crack avec dictionnaire est termin�; '+O+'La cl� WPA pour "' + ssid + '" n\'a pas �t� trouv�e dans le dictionnaire'
		...
		if index != len(WPA_CRACK) - 1:
			...
			menu= G+'   [c]ontinuer � cracker les autres handshakes ('+str(len(WPA_CRACK)-index-1)+' remaining)\n'
			menu+=R+'   [e]xit'
			
			print GR+'\n[+] '+W+'S�lectionnez une option dans le menu:'
			print menu
	...
	if DICT == '':
		for x in ATTACK:
			if TARGETS[x-1][2].startswith('WPA'):
				...
				print GR+'\n[+] '+W+'Afin de cracker des cl�s WPA, vous avez besoin d\''+O+'entrer un fichier dictionnaire'
				ent = 'blahnotafile'
				#try:
				while 1:
					...
					if ent == 'none' or ent == '"none"':
						...
						print R+'[!] Erreur! Chemin non trouv�: '+O+ent+R+'; Essayez encore SVP\n'
					else:
						DICT=ent
...
def attack_wep_all(index):
	...
	if weps[0]==False and weps[1]==False and weps[2]==False and weps[3]==False:
		print R+'[!] Aucune attaque WEP n\'a �t� s�lectionn�e; impossible d\'attaquer!'
	...
	try:
		...
		if client == THIS_MAC or is_shared(index) or CHANGE_MAC == False:
			...
			if not HAS_INTEL4965:
				...
				print GR+'[+] '+W+'D�marrage de '+O+'wlan1'+W+' sur le channel '+O+str(TARGETS[index][1])+W
				subprocess.call(['airmon-ng','start','wlan0',str(TARGETS[index][1])], \
							stdout=open(os.devnull,'w'), stderr=open(os.devnull,'w'))
				
				print GR+'[+] '+R+'D�marrage de '+W+'airodump-ng'
				# open airodump to capture packets
				cmd = ['airodump-ng','-w',TEMPDIR+'wep','-c',TARGETS[index][1], '--bssid',TARGETS[index][0], \
						'--output-format','csv,ivs',IFACE]
				proc_read = subprocess.Popen(cmd, stdout=open(os.devnull, 'w'), stderr=open(os.devnull, 'w'))
			...
			if faked:
				...
				print GR+'\r['+get_time(WEP_MAXWAIT,TIME_START)+'] '+G+'Fake authentication r�ussie :)       '
				if CHANGE_MAC == False:
					client=THIS_MAC
			else:
				# fake auth was unsuccessful (SKA?)
				print GR+'\r['+get_time(WEP_MAXWAIT, TIME_START)+'] '+R+'Fake authentication rat�e :(       '
				if EXIT_IF_NO_FAKEAUTH:
					...
					print GR+'['+get_time(WEP_MAXWAIT, TIME_START)+'] '+O+\
							'Continuer l\'attaque quand meme (les chances de succ�s sont faibles)'+W
			...
			subprocess.call(['ifconfig',IFACE,'up'])
			print GR+'['+get_time(WEP_MAXWAIT,TIME_START)+'] '+W+'MAC chang�e; l\'attaque continue'
			time.sleep(0.3)
		...
		if len(WPA_CRACK) > 0 and DICT != '' and DICT != 'none':
			...
			opts+=O+'s'+W
			if len(WPA_CRACK) == 1:
				menu=menu+O+'   pa[s]ser au cracking WPA (vous avez 1 handshake � cracker)\n'
			else:
				menu=menu+O+'   pa[s]ser au cracking WPA (vous avez '+str(len(WPA_CRACK))+' handshakes � cracker)\n'
	...
	for wepnum in xrange(0, len(weps)):
		if weps[wepnum]==True:
			...
			print GR+'['+get_time(WEP_MAXWAIT,TIME_START)+ \
				'] '+W+'Commence � cracker avec '+GR+wepname[wepnum]+W+' sur "'+G+TARGETS[index][8]+W+'"; '+GR+'Ctrl+C pour les options'
				...
				try:
					if proc_replay.poll() != None: # and wepnum != 0 and wepnum != 3:
						...
						if wepnum == 0 or wepnum == 3:
							print R+'\n['+get_time(WEP_MAXWAIT,TIME_START)+'] '+wepname[wepnum]+' Attaque �chou�e'
							break
						...
						if xor_file == '':
							...
							print R+'\n['+get_time(WEP_MAXWAIT,TIME_START)+\
									  '] Attaque �chou�e; '+O+'Impossible de g�n�rer le flux de cl�s'+W
							break
							...
							print GR+'\n['+get_time(WEP_MAXWAIT,TIME_START)+ \
									   '] '+G+'Flux de cl�s r�alis�, '+O+'fabrication avec packetforge-ng...'
							...
							if result.lower().find('Wrote packet'):
								...
								print GR+'['+get_time(WEP_MAXWAIT,TIME_START)+'] '+G+'Relance le flux de cl�s avec arp-replay...'
					...
					if os.path.exists(TEMPDIR+'wepkey.txt'):
						...
						CRACKED += 1
						print GR+'\n['+get_time(WEP_MAXWAIT,TIME_START)+'] '+G+'Cl� wep trouv�e pour "'+TARGETS[index][8]+'"!'
						print GR+'['+get_time(WEP_MAXWAIT,TIME_START)+'] '+W+'La cl� est "'+C + pw + W+'", sauv�e dans '+G+'log.txt'
						...
						if to_ascii(pw).find('non-print') == -1:
							logit('Cl� WEP crack�e pour "'+TARGETS[index][8]+'", la cl� est: "'+pw+'", en ascii: "' + to_ascii(pw) +'"')
						else:
							logit('Cl� WEP crack�e pour "'+TARGETS[index][8]+'", la cl� est: "'+pw+'"')
					...
					print 'IVs captur�s: '+G+ str(ivs) +W+' ...',
					ivsps = (ivs-oldivs) / 5
					...
					if len(WPA_CRACK) > 0 and DICT != '' and DICT != 'none':
						...
						opts+=O+'s'+W
						if len(WPA_CRACK) == 1:
							menu+=O+'   Pa[s]ser au crack du handshake (vous avez 1 handshake � cracker)\n'
						else:
							menu+=O+'   Pa[s]ser au crack des handshakes (vous avez '+str(len(WPA_CRACK))+' handshakes � cracker)\n'
				...
				if WEP_MAXWAIT != 0 and (time.time() - TIME_START) >= WEP_MAXWAIT:
					...
					for i in xrange(wepnum+1,len(weps)):
						if weps[wepnum] == True:
							...
							break
					if wcount > 0:
						print '\n'+R+'[+] de temps d\'attaque de '+wepname[wepnum]+' est d�pass�',
			# end of while loop
			print W
	...
	if stop_attack == False:
		...
		if started_crack:
			print GR+'[+] '+O+'Attaque rat�e!'+W+' impossible de cracker la cl� WEP (temps �coul�)'
		else:
			print GR+'[+] '+O+'Attaque rat�e!'+W+' impossible de g�nerer assez de IVs (temps �coul�)'
	...
	# change mac back
	if OLD_MAC != '':
		print GR+'[+] '+O+'Retour � l\'adresse MAC initiale...'+GR+OLD_MAC.lower()+O+'...'
		subprocess.call(['ifconfig',IFACE,'down'])
		subprocess.call(['macchanger','-m',OLD_MAC,IFACE], stdout=open(os.devnull,'w'))
		subprocess.call(['ifconfig',IFACE,'up'])
		OLD_MAC=''
		print GR+'[+] '+G+'Vous avez retrouv� votre adresse MAC normale'
...
def to_ascii(txt):
	...
	"""
	if len(txt) % 2 != 0:
		return '[Contiens des caract�res non affichable]'
	...
	if wrong == True:
		s=s+' [Contiens des caract�res non affichable]'
	return s
...
def attack_fakeauth_intel(index):
	...
	print GR+'[+] '+W+'Un d�lai de 30 secondes � partir de maintenant ...'
	
	proc_intel=pexpect.spawn(cmd)
	try:
		proc_intel.expect('State: ASSOCIATED -> COMPLETED', timeout=30)
		print GR+'[+] '+W+'Re�u: '+G+'Etat: Associ� -> Fini'+W
		return True
	except pexpect.TIMEOUT:
		print R+'[+] N\'a pas re�u '+O+'Etat: Associ� -> Fini'
		# kill the child process
		proc_intel.close(force=True)
	
	print R+'[!]        solution de contournement wpa_supplicant a �chou�!'
...
def attack_fakeauth(index):
	...
	# it's SKA (Shared Key Authentication). this is a BITCH
		...
		print GR+'['+get_time(AUTH_MAXWAIT,START_TIME)+'] '+O+'Passage � shared key authentication ...'
			...
			if os.path.exists(thexor):
				...
				if txt.lower().find('association successful') != -1:
					...
					print R+'['+sec2hms(AUTH_MAXWAIT-(time.time()-START_TIME)) + \
						  '] Fichier .xor invalide: "SKA Cass�?" Impossible de faire la fake authentication :('
					faked=False
				break
...
def attack_wpa(index):
	...
	if os.path.exists('hs/'+temp+'.cap'):
		...
		print GR+'[+] '+R+'Capture annul�e '+W+'car le fichier '+O+temp+'.cap'+G+' existe d�ja!'
	...
	try:
		...
		print GR+'['+sec2hms(WPA_MAXWAIT)+'] '+W+'D�marrage de la capture du handshake WPA'
		got_handshake=False
		...
		while time.time() - TIME_START < WPA_MAXWAIT or WPA_MAXWAIT == 0:
			...
			#txt=proc_crack.communicate()
				...
				print '\r'+GR+'['+get_time(WPA_MAXWAIT,TIME_START)+ \
						'] '+W+'Envoi de 3 paquets deauth; '+G+'handshake captur�!'+W+' sauv� dans "'+G+temp+'.cap'+W+'"'
				sys.stdout.flush()
				...
				if STRIP_HANDSHAKE:
					...
					if proc_pyrit.communicate()[0].strip() != '':
						...
						print GR+'['+get_time(WPA_MAXWAIT,TIME_START)+ \
								'] '+G+'Handshake'+W+' d�coup� avec '+G+'pyrit'+W
					...
					if len(cli) > 5:
						if cli[5].strip() == TARGETS[index][0]:
							...
							try:
								...
								print '\r'+GR+'['+get_time(WPA_MAXWAIT,TIME_START)+\
								              '] '+G+'Nouveau client ajout�: '+W+cli[0]+', '+G+'au total: '+str(len(wpa_clients))+' '
		...
		if len(WPA_CRACK) > 0 and DICT != '' and DICT != 'none':
			...
			opts+=O+'s'+W
			if len(WPA_CRACK) == 1:
				menu=menu+O+'   Pa[s]ser au crack du handshake WPA (vous avez 1 handshake � cracker)\n'
			else:
				menu=menu+O+'   Pa[s]ser au crack du handshake WPA (vous avez '+str(len(WPA_CRACK))+' handshakes � cracker)\n'
...
def gettargets():
	...
	try:
		...
		if ESSID == '':
			print GR+'[+] '+W+'Attente des cibles. Faites '+G+'Ctrl+C'+W+' lorsque vous �tes pret\n'
		elif ESSID == 'all' or ESSID.startswith('pow>'):
			time_wait=20
			if CHANNEL == '0':
				time_wait=30
			for i in xrange(time_wait, 0, -1):
				print GR+'\r[+] '+W+'Attendez '+G+str(i)+W+' secondes que les r�seaux soient affich�s. Faites '+O+'Ctrl+C'+W+' pour passer l\'attente ',
				sys.stdout.flush()
				time.sleep(1)
				parsetargets()
		...
		while 1:
			...
			if ESSID != '' and ESSID != 'all' and not ESSID.startswith('pow>'):
				if waiting==-1:
					for x in xrange(0, len(TARGETS)):
						if TARGETS[x][8].lower() == ESSID.lower():
							print GR+'\n[+] "'+G+ESSID+W+'" trouv�, attente d\'un client pendant '+G+'5 sec'+W+'...',
							sys.stdout.flush()
							ATTACK=[x+1]
							waiting=0
							break
					...
					for x in xrange(0, len(TARGETS)):
						if TARGETS[x][8].lower() == ESSID.lower():
							print GR+'\r[+] "'+G+ESSID+W+'" trouv�, attente d\'un client pendant '+G+str(5-waiting)+' sec'+W+'...',
							waiting += 1
							ATTACK=[x+1]
				...
				print '\r'+GR+'['+sec2hms(time.time()-TIME_START)+ \
						'] '+G+str(len(TARGETS))+W+' cibles et '+G+str(len(CLIENTS))+W+' clients trouv�s',
	...
	if ESSID == 'all':
		...
		for i in xrange(0, len(TARGETS)):
			try:
				if int(TARGETS[i][5]) >= power:
					print GR+'[+] '+W+'Ajout� � la liste d\'attaque: "'+G + TARGETS[i][8] + W+'" ('+G + TARGETS[i][5] + 'dB'+W+')'
					ATTACK.append(i+1)
			except ValueError:
		...
		if ATTACK==[]:
			print R+'[+] Il n\'y a pas de cibles avec un niveau de puissance sup�rieure � '+O + str(power) + 'dB'
			print R+'[+] Essayez de choisir un '+O+'seuil de puissance plus faible'
			print W
			subprocess.call(['rm','-rf',TEMPDIR])
			sys.exit(0)
		
		print GR+'[+] '+G+str(len(ATTACK))+W+' r�seaux cibl�s pour l\'attaque'
		return
	...
	print ''
	if len(TARGETS) == 0:
		print R+'[!] Pas de cibles trouv�es ! Soyez sur que '+O+'airodump-ng'+R+' fonctionne bien'
		print R+'[!] Le programme va se fermer'
		print W
		subprocess.call(['rm','-rf',TEMPDIR])
		sys.exit(0)
	
	print GR+'[+] '+W+'S�lectionnez le(s) '+G+'num�ro(s)'+W+' des/de la cible(s) que vous voulez attaquer:'
	for i in xrange(0, len(TARGETS)):
		# get power dB
		try:
	...
	print GR+'\n[+] '+W+'Pour s�lectionner plusieurs choix, utilisez le '+C+'- (tiret)'+W+' et la '+C+', (virgule)'+W+' pour s�parer'
	print GR+'[+] '+W+'Exemple: '+G+'1-3,5-6'+W+' va s�lectionner les cibles '+C+'1, 2, 3, 5, 6'
	print GR+'[+] '+W+'Pour attaquer tout les r�seaux, �crivez "'+G+'all'+W+'"'+G
	response = raw_input()
	
	ATTACK=stringtolist(response, len(TARGETS))
	if len(ATTACK) > 0:
		for x in ATTACK:
			print GR+'[+] '+W+'Ajout de "'+G + TARGETS[x-1][8] + W+'" � la liste d\'attaque'+W
...
def parsetargets():
	...
	try:
		...
		print R+'[+] SVP vous assurer d\'avoir aircrack-ng v1.1 ou sup�rieur'
	...
	if len(DEAUTH) > 0:
		msg='[Envoi de deauth cach�'
		if len(DEAUTH) > 1:
			msg+='s'
		msg+=']'
		print ''+O+' [Envoi de deauth cach�] '+W,
		for d in DEAUTH:
			sys.stdout.flush()
...
def stringtolist(s, most):
	...
	try:
		...
		print R+'[+] Erreur! entr�e invalide'+W
